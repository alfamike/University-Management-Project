...
class KaleidoClient {
    constructor() {
        ...
        if (!apiKey) {
            console.error('Must set environment variable "KALEIDO_API_KEY" to proceed');
            process.exit(1);
        }
        this.apiAuth = {
            headers: {
    ...
    async locateConsortium() {
        let result = await axios.get(`${this.kaleidoUrl}/consortia`, this.apiAuth);
        let consortium;
    ...
    async getCertFile(membershipId) {
        if (!this.config.organizations[membershipId]?.certificateAuthorities) {
            throw new Error(`No certificate authorities defined for ${membershipId}`);
        }
        ...
        // Ensure directory before writing
        await fs.ensureFile(fileName);
        await fs.createFile(fileName, pem);
        return fileName;
    }
